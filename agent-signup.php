...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $checkStmt = $conn->prepare("SELECT * FROM agents WHERE email = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO agents (name, email, password) VALUES (?, ?, ?)");
            $insertStmt->bind_param("sss", $name, $email, $hashedPassword);
            
            if ($insertStmt->execute()) {
                header("Location: admin-manageagent.php?success=1");
                exit;
            } else {
...
<body>
  <div class="container <?php echo !empty($errors) ? 'shake' : '' ?>">
    ...
    <form action="agent-signup.php" method="POST" id="signupForm">
