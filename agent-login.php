    ...
    if (empty($errors)) {
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $agent['password'])) {
                ...
                header("Location: agent-dashboard1.php");
                exit;
            } else {
                $errors['password'] = "Incorrect password";
            }
        } else {
            $errors['email'] = "Agent account not found";
        }
        $stmt->close();
    }
    $conn->close();
...
<head>
  ...
  <title>Agent Login - Ticketing System</title>
...
<body>
  <form class="login-container <?php echo !empty($errors) ? 'shake' : '' ?>" method="POST">
    ...
    <div class="note">Only registered agents can log in</div>
  </form>
